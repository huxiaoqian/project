...
##                    for user in users():
##                        print 'yuan'
##                        print user['_id'],user['profile_image_url'],user['description']
...
def topic_rank():
    ...
    topic_key_user_list = []
    for i in range(0,len(topic_info)):
        number, users = xapian_search_user.search(query={'_id': topic_info[i]}, fields=['_id','name','location','friends_count','followers_count','statuses_count','description'])
        if not number:
            continue
        for user in users():
            print 'yuan',user['statuses_count'],user['description']
            topic_key_user_list.append({'id':user['_id'],'name':user['name'],'location':user['location'],'followers_count':user['followers_count'],'bi_followers_count':user['friends_count'],'statuses_count':user['statuses_count'],'description':user['description']})
    ...
    for user in topic_key_user_list:
        if user:
            ...
            if n > startoffset:
                if n > endoffset:
                    break
                news.append({'id':user['id'],'name':user['name'],'location':user['location'],'followers_count':user['followers_count'],'bi_followers_count':user['bi_followers_count'],'statuses_count':user['statuses_count'],'description':user['description']})
    total_pages = limit / countperpage + 1
    return json.dumps({'news': news, 'pages': total_pages})
