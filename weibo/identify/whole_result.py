...
def whole_caculate(date, identifyWindow, identifyMethod, top_n):
    ...
    if identifyWindow == 1:#时间窗为1，直接从数据库中读取
        items = db.session.query(WholeIdentification).filter((WholeIdentification.identifyMethod==identifyMethod)&(WholeIdentification.identifyWindow==identifyWindow)&(WholeIdentification.identifyDate==current_date)&(WholeIdentification.rank<=top_n)).all()
        if len(items):
            for item in items:
                rank = int(item.rank)
                uid = str(item.userId)
        ...
        for i in range(0,identifyWindow):#遍历所有时间
            ...
            items = db.session.query(WholeIdentification).filter((WholeIdentification.identifyMethod==identifyMethod)&(WholeIdentification.identifyWindow==1)&(WholeIdentification.identifyDate==begin)).all()
            if len(items):
                ...
                continue
        rank_data = whole_rank(active,important,follower,top_n,identifyMethod)
...
def whole_rank(active,important,follower,top_n,identifyMethod):#全网排序

    if identifyMethod == 'active':
        print 'whole active rank'
        count = 0
