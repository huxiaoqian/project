...
from flask import url_for, render_template, request, abort, flash, make_response
from model import *
...
search_weibo_test = XapianSearch(path='/opt/xapian_weibo/data/faked/', name='master_timeline_weibo', schema_version=2)
...
def getHotStatus(limit_num=5):
    statuses = db.session.query(HotStatus).order_by(HotStatus.repostsCount.desc()).limit(limit_num)
    status_hot = []             
...
@app.route("/topic_ajax_trend/", methods = ["GET","POST"])
...
@app.route("/topic_ajax_spatial/", methods=['GET', 'POST'])
...
@app.route("/topic_ajax_path/", methods=['GET', 'POST'])
def topic_ajax_path():
    if request.method == "GET":
        return render_template('propagate/ajax/topic_retweetpath.html')

@app.route("/topic_ajax_userfield/", methods=['GET', 'POST'])
...
@app.route("/showresult_single/<mid>/", methods = ["GET","POST"])
...
@app.route("/single_ajax_trend/", methods = ["GET","POST"])
...
@app.route("/single_ajax_spatial/", methods=['GET', 'POST'])
...
@app.route("/single_ajax_path/", methods=['GET', 'POST'])
def single_ajax_path():
    if request.method == "GET":
        return render_template('propagate/ajax/single_retweetpath.html')

@app.route("/single_ajax_userfield/", methods=['GET', 'POST'])
...
@app.route("/add_material/<mid>/", methods = ["GET","POST"])
