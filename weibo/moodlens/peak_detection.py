...
>>new=[9,8,7,6,5]
>>rank=[0,4,1,3,2]
即对lis进行降序排列，并利用rank记录new中每个元素的初始位置
'''
...
def filter_continuous(peaks):
        ...
        return new_peaks
'''
filter_continuous 用例
peaks=[4,9,5,6,7]
>>filter_continuous [5,6,7]
>>new_peaks=[4,9]
即对peaks当中连续的点去掉
'''
...
def filter_flat(lis,peaks):
        ...
        for pk in peaks:
                if pk < 0 or pk > len(lis)-1:
                        filters.append(pk)   #排除掉pk<0，pk>len(lis)-1的pk
                        continue
                vr = sentiment_variation(lis,pk)
                if vr == 0:
                        filters.append(pk)   #排除掉pk的vr为0的情况
        peaks = [pk for pk in peaks if pk not in filters]
        return peaks,filters     #选出最终的最高点以及奇异值
...
def find_topN(lis,topN):
        new,rank = sort_list(lis)  #对其进行排序，然后在此后按其顺序寻找极值点       
        peak_x = []
        cursor = 0  
        for y in new:
                if rank[cursor]!=0 and rank[cursor]!=len(new)-1:  #当下标位置在范围内时
                        if y >= lis[rank[cursor]+1] and y >= lis[rank[cursor]-1]:  #当y值大于其两边的值，就说明他是极值点
                                peak_x.append(rank[cursor])
                cursor += 1
        peaks = filter_continuous(peak_x)
        return peaks[:topN]   #返回极值点前n个
