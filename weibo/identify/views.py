...
def whole():
    ...
    if request_method == 'GET':
        ...
        if action == 'rank':
            data = read_current_results(top_n, topic_id=None, r='whole', m=rank_method, w=window_size)
            return json.dumps({'status': 'current finished', 'data': data})
        elif action == 'previous_rank':
            previous_data = read_previous_results(top_n, topic_id=None, r='area', m=rank_method, w=window_size)
...
def area():
    ...
    if request_method == 'GET':
        ...
        if action == 'rank':
            data = read_current_results(top_n, topic_id=topic_id, r='area', m=rank_method, w=window_size)
            return json.dumps({'status': 'current finished', 'data': data})
        elif action == 'previous_rank':
            previous_data = read_previous_results(top_n, topic_id=topic_id, r='area', m=rank_method, w=window_size)
            return json.dumps({'status': 'previous finished', 'data': previous_data})
        elif action == 'check':
            ...
            return render_template('identify/area.html', field=field, keywords=keywords, top_n=top_n, page_num=page_num, hadoop=hadoop)
        else:
            abort(404)
    else:
        abort(404)
