...
if __name__ == '__main__':
    ...
    # init bdb
    profile_domain_keywords_hash_db = db.DB(db_env)
    profile_domain_keywords_hash_db.open('profile_domain_keywords_hash', None, db.DB_HASH, db.DB_CREATE)
    
    # init leveldb
    now_datestr = sys.argv[1]
    profile_domain_keywords_leveldb = leveldb.LevelDB(os.path.join(LEVELDBPATH, 'linhao_profile_domain_rtkeywords_%s' % now_datestr),
                                                      block_cache_size=8 * (2 << 25), write_buffer_size=8 * (2 << 25))
    profile_domain_keywords()

    profile_domain_keywords_hash_db.close()
    db_env.close()
