...
def sentimentRealTime(end_ts, during, method='whole', calc='count', query=None, w_limit=TOP_WEIBOS_LIMIT, k_limit=TOP_KEYWORDS_LIMIT):
    emotions_count = {}
    emotions_kcount = {}
    emotions_weibo = {}
...
def maintain_topic_sentiment(start_ts, end_ts):
	topics = _all_topics(iscustom=True)
	during = end_ts - start_ts

	for topic in topics:
		topicname = topic.topic
		results = sentimentRealTime(end_ts, during, method='topic', calc='all', query=topicname)


if __name__ == '__main__':
	start_ts = CRON_START_TS
	end_ts = datetime2ts('2013-10-01')
	maintain_topic_sentiment(start_ts, end_ts)